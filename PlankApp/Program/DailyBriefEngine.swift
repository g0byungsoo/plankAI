...
enum DailyBriefEngine {
    ...
    struct Brief: Equatable {
        ...
        var clause: String = ""
        /// v25 ship — true when `second` carries last night's tomorrow
        /// intention back. The payout half of the intention loop;
        /// categorical, so the morning read can report it honestly.
        var carriesIntention: Bool = false
    ...
    struct Context: Equatable {
        ...
        // p79 — THE FELT WEEK reaches the morning (r2's #1 moment:
        // day 5-7, "why am I so hungry — is it failing?"). The
        // event-anchored cycle position, single-dose rhythms only;
        // every field defaults to silence.
        /// Day of the dose cycle (1 = dose day), when a position
        /// honestly exists.
        var doseCycleDay: Int? = nil
        /// True exactly on the FIRST waning day — the clause speaks
        /// once per cycle, never three mornings running.
        var doseWaningOpens: Bool = false
        /// Her own recorded pattern: the typical food-noise onset
        /// day when the signature holds (≥3 consecutive cycles,
        /// clustered — MedicationPatternEngine.foodNoiseSignature).
        var foodNoiseTypicalDay: Int? = nil
    }
    ...
    private static func cascade(for ctx: Context) -> Brief {
        ...
        if ctx.programDay == 1 {
            return Brief(
                ...
                second: "start with one plate: add the last thing you ate.",
                clause: "day_one"
            )
        }
        ...
        if ctx.programDay >= 2, ctx.programDay <= 7,
           ...
           ctx.yesterdayPlateCount >= 1, !ctx.numericSuppressed {
            ...
            if ctx.programDay == 2 {
                return Brief(
                    line: "day two. \(plateWord) logged yesterday.",
                    italic: [plateWord],
                    ...
                    second: ctx.yesterdayProteinG.flatMap { p in
                        ...
                        return ctx.proteinTargetG.map {
                            "about \(p) g of protein. today's goal is \($0) g."
                        } ?? "about \(p) g of protein."
                    },
                    mechanism: forming,
                    clause: "day_two"
                )
        ...
        // 3.8 — p79 THE FELT WEEK: the back half of the dose week
        //       opens (r2's #1 moment: "why am I so hungry — is it
        //       failing?", the question the category outsources to
        //       Reddit). Once per cycle, on its first day — which is
        //       why it stands ABOVE the trend receipt: the trend line
        //       repeats tomorrow morning; this morning doesn't.
        //       Safety flags and the once-ever win still lead. Her
        //       own recorded pattern speaks when the record holds
    ...
    private static func archetypeBrief(_ ctx: Context) -> Brief {
        ...
        case .balanced:
            let lines: [(String, [String])] = [
                ("a normal day. check off what fits.", ["normal"]),
                ("a normal day. small things, checked off.", ["small things"]),
            ]
            let pick = lines[seedIndex % lines.count]
            return Brief(
        ...
        case .rest:
            if ctx.glp1Cohort == .postGlp1 {
                return Brief(
                    line: "rest day. recovery counts too.",
                    italic: ["recovery"],
            ...
            return Brief(
                line: "rest day. one minute of breathing, that's it.",
                italic: ["one minute"],
