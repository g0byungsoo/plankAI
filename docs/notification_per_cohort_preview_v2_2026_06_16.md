# Per-cohort notification preview — v2 (revised)

**Date:** 2026-06-16
**Supersedes:** `docs/notification_per_cohort_preview_2026_06_16.md`
**Source spec:** `docs/notification_system_spec_2026_06_16.md`

## Founder directives applied in this revision

1. **ONE trial-end reminder only** (T-24h). Dropped the T-30h soft tap and T-2h disclosure. Apple's own system push handles redundant disclosure.
2. **No spam.** Dropped Day 2 value-spotlight. Kept Day 1 morning as the one re-engagement push for users who haven't opened the app.
3. **No GLP-1 feature promises.** Stripped "protein floor," "food noise piece," "food noise quieting," "post-shot rhythm," "keep-it-off curriculum" — features don't exist yet. Cohort signal preserved in title (identity acknowledgment); bodies point only to what ships today: **lessons** (JeniMethod), **breath cards** (breathwork), **becoming** (analytics tab), **your plate** (food rail).

## Push count by scenario

| Scenario | Trial-week count |
|---|---|
| Engaged (shownUp ≥ 2 by Day 1) | 2-3 pushes |
| Ambivalent (shownUp == 1) | 2-3 pushes |
| Disengaged (shownUp == 0) | 3-4 pushes |

All under the 5/wk Airbridge ceiling.

## Variables

- `{opener}` = `"{userName}, "` (lowercased) when set; `""` otherwise.
- `{n}` = integer count (shown-up days or program days).
- `{coachName}` = Jeni / Sam / Kira based on `voicePreference`.
- `{tail}` = `" {name}"` when name set, `""` otherwise (milestones only).
...
**Day 1** — *if Mon/Wed/Fri at bucket-hour:*
- 🔄 **Daily reminder** (voice-routed — see Universal section)

**Day 2** — *if Mon/Wed/Fri at bucket-hour:* same Daily reminder
...
If shownUp == 0 (cold-zone):
- Title: *the rhythm is here when you are.*
- Body: `your trial converts tomorrow. lessons, breath cards, and your becoming. manage anytime in iOS settings.`
...
### Trial week (only deltas from Cohort A)

**Day 1 morning push** *(if shownUp == 0):*
- Title: *day one, alongside the shot.*
- Body: `{opener}five minutes today. that's how the rhythm begins ♥`

**Day 3 T-24h shownUp == 0:**
- Title: *the daily piece is here.*
- Body: `your trial converts tomorrow. lessons, breath cards, and your becoming. manage anytime in iOS settings.`

(shownUp ≥ 2 and shownUp == 1 branches are universal — same as Cohort A.)
...
**Day 3 T-24h shownUp == 0:**
- Title: *the rhythm is here when you are.* (same as Cohort A — the rhythm framing already fits the keep-it-off identity)
- Body: `your trial converts tomorrow. lessons, breath cards, and your becoming. manage anytime in iOS settings.`
...
## 🌐 Universal pushes (same for all 4 cohorts; no changes from v1)

These were already cohort-agnostic and reference no GLP-1 features.

### Daily reminder (post-week-1, daily; voice-routed)
...
The "becoming-line" is personalized to her `identityFeeling`:
...
### Sunday recap — Sunday 5pm (or bucket-hour Sunday if we pick that option)
